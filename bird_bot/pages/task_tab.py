...
from utils import (get_profile, get_proxy, BirdLogger,
	return_data, write_data,
...
class TaskThread(QtCore.QThread):
	...
	def set_data(self,task_id,site,product,monitor_proxies,
	...
	def run(self):
		if self.site == "Walmart":
			...
			GameStop(self.task_id, self.status_signal, self.image_signal, self.product, self.monitor_proxies,
				self.monitor_delay, self.error_delay, self.max_price, self.max_quantity, self.account)
...
class TaskTab(QtWidgets.QWidget):
	def __init__(self,task_id, site, product, monitor_proxies, monitor_delay, error_delay,
		max_price, max_quantity, account, stop_all, scroll_content, parent=None):
	...
	def update_status(self,msg):
		...
		if msg["status"] == "idle":
			...
			if settings.buy_one:
				self.stop_all()
			self.parent.checkouts_count.setText(str(int(self.parent.checkouts_count.text())+1))
		elif msg["status"] == "carted":
			self.status_label.setStyleSheet("color: rgb(163, 149, 255);")
			logger.alt(self.task_id,msg["msg"])
			self.parent.carted_count.setText(str(int(self.parent.carted_count.text())+1))
	...
	def edit(self,event):
		self.edit_dialog = NewTask(self, True, self.task_id)
		self.edit_dialog.show()
		if self.edit_dialog.exec_() == QtWidgets.QDialog.Accepted:
			self.edit_dialog.update_task()
