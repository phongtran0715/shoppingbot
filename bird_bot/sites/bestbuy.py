...
class BestBuy:
	...
	def __init__(self, task_id, status_signal, image_signal, product,
		monitor_proxies, monitor_delay, error_delay, account):
		...
		self.shopping_proxies = get_proxy(account_item['proxy'])
		if self.shopping_proxies is not None and self.shopping_proxies != "":
			print("Shopping proxy : {}".format(self.shopping_proxies))
			self.session.proxies.update(self.shopping_proxies)
